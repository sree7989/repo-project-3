...
const Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCountry("");
        setMessage("");
        router.push(landingPage + (landingPage.endsWith("/") ? "" : "/") + "thankyou");
      } else {
        setFormStatus("error");
      }
    } catch (error) {
  ...
  if (!landingPage) return null; // For hydration fix
  ...
  return (
    <div className="bg-white p-4 py-2 rounded-lg shadow-sm max-w-md mx-auto w-full h-[35rem] md:h-[32rem] lg:h-[32rem] tablet:h-[35rem] shadow-orange-300 mb-6 lg:mb-2">
      <h2 className="text-2xl font-bold text-center uppercase text-gray-800 mt-0 lg:mt-3">
